...
from models.webhook import WebhookEndpoint, WebhookDelivery, WebhookEvent
...
class WebhookService:
        ...
        try:
            ...
            event_bus.publish(
                DomainEvent(
                    ...
                    payload={
                        "event": event,
                        "endpoints_count": len(matching_endpoints),
                    },
                )
            )
        except Exception:
    ...
    def get_deliveries_query(
        ...
        if status == "success":
            query = query.filter(
                WebhookDelivery.status_code >= 200, WebhookDelivery.status_code < 300
            )
        elif status == "failed":
            query = query.filter(
