...
from models import db, Project, SysModel, SysField
from modules.builder.models import Block as BlockModel
...
- ID: {project.id}"""
    ...
    def _get_workflows_info(self) -> str:
        ...
        workflows = Workflow.query.filter_by(projectId=self.projectId).all()
...
def get_project_context(projectId: int) -> str:
    """
    ...
    """
    builder = AIContextBuilder(projectId)
    return builder.build_context()


def get_conversation_context(projectId: int, limit: int = 5) -> str:
    """
    ...
    try:
        ...
        conversations = (
            AIConversation.query.filter_by(projectId=projectId, was_successful=True)
