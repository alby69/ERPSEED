"""
Analytics Command Handlers - Handle commands for Charts and Libraries.
"""
import logging
...
from backend.analytics_service.domain.events import ChartCreatedEvent, ChartDeletedEvent
...
class GetDefaultLibraryHandler(QueryHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        ...
        if not result:
            return CommandResult.ok({"library_name": "chartjs", "is_default": True, "is_active": True})
        return CommandResult.ok(result)
