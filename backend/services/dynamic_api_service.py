...
class DynamicApiService(BaseService):
                            ...
                            if line.get('_destroy'):
                                if line.get('id'):
                                    self.db.session.execute(
                                        delete(target_table).where(target_table.c.id == line['id'])
                                    )
                            elif line.get('id'):
                                self.db.session.execute(
                                    update(target_table).where(target_table.c.id == line['id']).values(line)
                                )
                            else:
                                if 'id' in line:
                                    del line['id']
    ...
    def list_records(self, project_id, model_name, page=1, per_page=10):
        ...
        if not sys_model:
            from flask_smorest import abort
            abort(404, message=f"Model '{model_name}' not found in project.")
