...
class PurchaseOrderRepository:
    ...
    def create(self, data: Dict[str, Any]) -> Dict[str, Any]:
        PurchaseOrder = self._get_order_class(); PurchaseOrderLine = self._get_line_class()
        order = PurchaseOrder()
        ...
        order.supplier_id = data.get("supplier_id", 0); order.notes = data.get("notes", ""); order.status = "draft"
        if data.get("expected_date"): order.expected_date = datetime.strptime(data["expected_date"], "%Y-%m-%d").date()
        total = 0
        for line_data in data.get("lines", []):
            line = PurchaseOrderLine(); line.tenant_id = order.tenant_id
