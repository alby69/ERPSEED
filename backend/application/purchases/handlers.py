"""
Purchases Handlers - Handle commands and return results.
"""
import logging
...
from backend.domain.purchases import (
...
class CreatePurchaseOrderHandler(CreateHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, CreatePurchaseOrderCommand): return CommandResult.error(f"Invalid command type: {type(command)}")
        ...
        try:
            ...
            if self.event_bus: self.event_bus.publish(PurchaseOrderCreatedEvent(result["id"], result, command.tenant_id))
...
class UpdatePurchaseOrderHandler(UpdateHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, UpdatePurchaseOrderCommand): return CommandResult.error(f"Invalid command type: {type(command)}")
        ...
        try:
            ...
            if self.event_bus and result: self.event_bus.publish(PurchaseOrderUpdatedEvent(command.entity_id, result["old"], result["new"], command.tenant_id))
...
class DeletePurchaseOrderHandler(DeleteHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, DeletePurchaseOrderCommand): return CommandResult.error(f"Invalid command type: {type(command)}")
        if not command.entity_id: return CommandResult.error("Order ID is required")
...
class ConfirmPurchaseOrderHandler(CommandHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, ConfirmPurchaseOrderCommand): return CommandResult.error(f"Invalid command type: {type(command)}")
        existing = self.repository.find_by_id(command.entity_id, command.tenant_id)
        if not existing: return CommandResult.error(f"Order not found: {command.entity_id}")
        ...
        try:
            ...
            if self.event_bus and result: self.event_bus.publish(PurchaseOrderConfirmedEvent(command.entity_id, result, command.tenant_id))
            return CommandResult.ok(result)
        except Exception as e:
...
class ReceivePurchaseOrderHandler(CommandHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, ReceivePurchaseOrderCommand): return CommandResult.error(f"Invalid command type: {type(command)}")
        existing = self.repository.find_by_id(command.entity_id, command.tenant_id)
        if not existing: return CommandResult.error(f"Order not found: {command.entity_id}")
        ...
        try:
            ...
            if self.event_bus and result: self.event_bus.publish(PurchaseOrderReceivedEvent(command.entity_id, result, command.tenant_id))
            return CommandResult.ok(result)
        except Exception as e:
...
class CancelPurchaseOrderHandler(CommandHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, CancelPurchaseOrderCommand): return CommandResult.error(f"Invalid command type: {type(command)}")
        existing = self.repository.find_by_id(command.entity_id, command.tenant_id)
        if not existing: return CommandResult.error(f"Order not found: {command.entity_id}")
        ...
        try:
            ...
            if self.event_bus and result: self.event_bus.publish(PurchaseOrderCancelledEvent(command.entity_id, result, command.tenant_id))
            return CommandResult.ok(result)
        except Exception as e:
...
class GetPurchaseOrderHandler(QueryHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, GetPurchaseOrderCommand): return CommandResult.error(f"Invalid command type: {type(command)}")
        if not command.entity_id: return CommandResult.error("Order ID is required")
...
class ListPurchaseOrdersHandler(QueryHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, ListPurchaseOrdersCommand): return CommandResult.error(f"Invalid command type: {type(command)}")
        try:
