...
from backend.schemas import SysChartSchema, SysDashboardSchema, ChartLibraryConfigSchema
from backend.shared.utils.utils import get_table_object
from backend.services import DynamicApiService
from backend.services.generic_service import generic_service
from backend.shared.utils.utils import paginate, serialize_value
...
def get_repos():
    return {
        'chart': ChartRepository(db),
        'library': ChartLibraryRepository(db),
    }
...
class ChartLibraryList(MethodView):
    ...
    def get(self):
        ...
        repo = get_repos()['library']
        items = ChartLibraryConfig.query.all()
        return [ChartLibraryConfigSchema().dump(item) for item in items]
    ...
    def post(self, library_data):
        ...
        try:
            item = ChartLibraryConfig.query.filter_by(library_name=library_data.get('library_name')).first()
            if item:
                for key, value in library_data.items():
                    setattr(item, key, value)
            else:
                item = ChartLibraryConfig(**library_data)
                db.session.add(item)
            db.session.commit()
            return ChartLibraryConfigSchema().dump(item), 201
...
class ChartLibraryResource(MethodView):
    ...
    def get(self, library_id):
        item = ChartLibraryConfig.query.get_or_404(library_id)
        return ChartLibraryConfigSchema().dump(item)
...
class ChartLibraryDefault(MethodView):
    ...
    def get(self):
        ...
        return ChartLibraryConfigSchema().dump(default_lib)
...
class SysChartList(MethodView):
    ...
    def get(self):
        ...
        return [SysChartSchema().dump(item) for item in items]
    ...
    def post(self, chart_data):
        try:
            item = SysChart(**chart_data)
            db.session.add(item)
            db.session.commit()
            return SysChartSchema().dump(item), 201
        except Exception as e:
            db.session.rollback()
            abort(400, message=str(e))
...
class SysChartResource(MethodView):
    ...
    def get(self, chart_id):
        item = SysChart.query.get_or_404(chart_id)
        return SysChartSchema().dump(item)
