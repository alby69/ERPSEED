"""
Sales Command Handlers - Handle commands and return results.
"""
import logging
from typing import Optional

from backend.shared.handlers import (
    CommandHandler, CommandResult, CreateHandler, UpdateHandler, DeleteHandler, QueryHandler
)
from backend.shared.commands import Command

from backend.sales_service.application.commands import (
...
from backend.sales_service.domain import (
...
from backend.sales_service.infrastructure.repository import SalesOrderRepository
...
class CreateSalesOrderHandler(CreateHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, CreateSalesOrderCommand):
        ...
        if not customer:
            return CommandResult.error("Customer not found")
...
class UpdateSalesOrderHandler(UpdateHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, UpdateSalesOrderCommand):
            return CommandResult.error(f"Invalid command type: {type(command)}")
        
        if not command.entity_id:
            return CommandResult.error("Order ID is required")
        
        existing = self.repository.find_by_id(command.entity_id, command.tenant_id)
        if not existing:
            return CommandResult.error(f"Order not found: {command.entity_id}")
...
class DeleteSalesOrderHandler(DeleteHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, DeleteSalesOrderCommand):
            return CommandResult.error(f"Invalid command type: {type(command)}")
        ...
        if not check["can_delete"]:
            return CommandResult.error(check["reason"])
...
class ConfirmSalesOrderHandler(CommandHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, ConfirmSalesOrderCommand):
            return CommandResult.error(f"Invalid command type: {type(command)}")
        ...
        if existing["status"] != "draft":
            return CommandResult.error("Only draft orders can be confirmed")
...
class CancelSalesOrderHandler(CommandHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, CancelSalesOrderCommand):
            return CommandResult.error(f"Invalid command type: {type(command)}")
        ...
        if existing["status"] in ["completed", "cancelled"]:
            return CommandResult.error("Order cannot be cancelled in current status")
...
class GetSalesOrderHandler(QueryHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, GetSalesOrderCommand):
            return CommandResult.error(f"Invalid command type: {type(command)}")
...
class ListSalesOrdersHandler(QueryHandler):
    ...
    def handle(self, command: Command) -> CommandResult:
        if not isinstance(command, ListSalesOrdersCommand):
            return CommandResult.error(f"Invalid command type: {type(command)}")
