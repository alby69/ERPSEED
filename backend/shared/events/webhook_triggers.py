...
def trigger_webhook(event: str, data: dict, async_mode: bool = True):
    ...
    try:
        ...
        print(f"EventBus publish failed: {e}", file=sys.stderr)
...
def on_model_updated(model):
    ...
    trigger_webhook(
        "model.updated", {"id": model.id, "name": model.name, "title": model.title}
    )
...
def on_record_deleted(model_name, record_id, project_id):
    ...
    trigger_webhook(
        "record.deleted",
        {"model": model_name, "record_id": record_id, "project_id": project_id},
    )
