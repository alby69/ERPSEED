...
def _get_model_column(model, name):
    """Get a SQLAlchemy column from a model by name, handling accented/unusual names."""
...
def apply_filters(query, model, search_fields):
    ...
    if q and search_fields:
        ...
        for field in search_fields:
            col = _get_model_column(model, field)
            if col is not None:
...
def apply_sorting(query, model, default_sort_column=None):
    ...
    if sort_by:
        column = _get_model_column(model, sort_by)
    ...
    if field.type == "relation" and field.options:
        try:
            ...
            if target_table and target_table.isidentifier():
                target = (
                    f'"{schema}"."{target_table}"' if schema else f'"{target_table}"'
                )
...
def generate_schema_diff_sql(sys_model, db_engine, schema=None):
    ...
    model_fields = {
        ...
        if f.type not in ["formula", "summary", "lookup", "calculated"]
    }
    type_mapping = _get_type_mapping()
    ...
    for col_name in db_column_names - model_field_names:
        sql_commands.append(
            f'ALTER TABLE {table_identifier} DROP COLUMN "{col_name}" CASCADE;'
        )
    ...
    for field_name in model_field_names.intersection(db_column_names):
        ...
        if field.required and is_db_nullable:
            sql_commands.append(
                f'ALTER TABLE {table_identifier} ALTER COLUMN "{field_name}" SET NOT NULL;'
            )
        elif not field.required and not is_db_nullable:
            sql_commands.append(
                f'ALTER TABLE {table_identifier} ALTER COLUMN "{field_name}" DROP NOT NULL;'
            )
        ...
        if target_type_str:
            ...
            if (
                target_base_type not in current_db_type
                and current_db_type not in target_type_str
            ):
                ...
                sql_commands.append(
                    f'ALTER TABLE {table_identifier} ALTER COLUMN "{field_name}" TYPE {target_type_str} {using_clause};'
                )
...
def get_table_object(model_name, schema=None):
    ...
    try:
        ...
        abort(
            404,
            message=f"Table '{model_name}' not found in the database. Please generate it first.",
        )
